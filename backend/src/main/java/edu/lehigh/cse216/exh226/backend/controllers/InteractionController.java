...
public class InteractionController {
    ...
    public InteractionController(Connection mConnection) throws SQLException {
        mSelectAll = mConnection.prepareStatement("SELECT * FROM interaction ORDER BY PID DESC");
        mSelectOne = mConnection.prepareStatement("SELECT * FROM interaction WHERE PID=?");
        mUpdateOne = mConnection.prepareStatement("UPDATE interaction SET LoD = ? WHERE PID = ?");
    ...
    public ArrayList<Interaction> selectAll() {
        ...
        try {
            ...
            while (rs.next()) {
                res.add(new Interaction(rs.getInt("PID"), rs.getString("UIID"), rs.getBoolean("LoD")));
            }
            rs.close();
            return res;
        } catch (SQLException e) {
    ...
    public Interaction selectOne(int PID) {
        ...
        try {
            ...
            if (rs.next()) { // Why do we do rs.next()?
                res = new Interaction(rs.getInt("PID"), rs.getString("UIID"), rs.getBoolean("LoD"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
